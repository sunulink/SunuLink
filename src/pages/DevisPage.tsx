...
const DevisPage = () => {
  ...
  useEffect(() => {
    emailjs.init("ShXDBB_RTc_F-EWm1");
  }, []);
  ...
  const validateEtape = (etape: number): boolean => {
    switch (etape) {
      case 1:
        return !!(formData.civilite && formData.nom && formData.prenom && formData.email && formData.telephone);
      case 2:
      ...
      <div className="min-h-screen bg-white">
        ...
        <main className="pt-32 pb-20">
          <div className="container mx-auto max-w-3xl px-6">
            <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-3xl p-12 text-center text-white shadow-2xl">
              <CheckCircle className="w-20 h-20 mx-auto mb-6" />
              <h1 className="text-4xl font-black mb-4">Demande envoyée !</h1>
              <p className="text-xl opacity-90 mb-8">Merci {formData.prenom} ! Nous vous recontacterons très bientôt.</p>
              <Button onClick={() => window.location.href = "/"} className="bg-white text-green-600 hover:bg-gray-100 font-bold py-4 px-8 rounded-xl text-lg">
                Retour à l'accueil
              </Button>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="pt-32 pb-20">
          ...
          <div className="container mx-auto max-w-4xl flex items-center justify-between">
            {etapes.map((etape, index) => (
              <div key={etape.numero} className="flex items-center">
                <div className={`flex flex-col items-center ${etapeActuelle >= etape.numero ? "text-sunuBlue" : "text-gray-400"}`}>
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center ${etapeActuelle >= etape.numero ? "bg-sunuBlue text-white shadow-lg" : "bg-gray-200 text-gray-500"}`}>
                    <etape.icon className="w-5 h-5" />
                  </div>
                  <span className="text-xs mt-2 font-semibold hidden sm:block">{etape.titre}</span>
                </div>
                {index < etapes.length - 1 && (
                  <div className={`w-8 sm:w-16 md:w-24 h-1 mx-2 rounded ${etapeActuelle > etape.numero ? "bg-sunuBlue" : "bg-gray-200"}`} />
                )}
              </div>
            ))}
          </div>
        ...
        {/* Form Logic */}
        <section ref={formSectionRef} className="px-6 pb-16">
          <div className="container mx-auto max-w-4xl">
            <form onSubmit={envoyerDevis}>
              {/* Étape 1 */}
              {etapeActuelle === 1 && (
                <div className="bg-sunuBlue rounded-3xl p-8 text-white shadow-2xl">
                  <h2 className="text-2xl font-black mb-6 flex items-center gap-3"><User /> Informations</h2>
                  <div className="space-y-6">
                    <RadioGroup value={formData.civilite} onValueChange={(v) => setFormData(p => ({...p, civilite: v}))} className="flex gap-4">
                      {["Monsieur", "Madame"].map(o => (
                    ...
                    <Input name="telephone" value={formData.telephone} onChange={handleInputChange} placeholder="Téléphone" className="bg-white/10 border-white/30 text-white" required />
                  </div>
                </div>
              )}
              ...
              {/* Étape 2 */}
              {etapeActuelle === 2 && (
                <div className="bg-sunuOrange rounded-3xl p-8 text-white shadow-2xl">
                  <h2 className="text-2xl font-black mb-6 flex items-center gap-3"><Briefcase /> Votre Projet</h2>
                  <div className="space-y-8">
                    {Object.entries(categoriesProjets).map(([cat, services]) => (
                      <div key={cat}>
                        <h3 className="font-bold border-b border-white/30 mb-3">{cat}</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {services.map(s => (
                            <label key={s} className="flex items-center space-x-3 bg-white/10 p-3 rounded-lg cursor-pointer">
              ...
              {/* Étape 3 */}
              {etapeActuelle === 3 && (
                <div className="bg-purple-600 rounded-3xl p-8 text-white shadow-2xl">
                  <h2 className="text-2xl font-black mb-6 flex items-center gap-3"><FileText /> Détails</h2>
                  <div className="space-y-6">
                    <Textarea name="descriptionProjet" value={formData.descriptionProjet} onChange={handleInputChange} placeholder="Décrivez votre projet..." rows={6} className="bg-white/10 border-white/30 text-white" required />
                    <div className="grid grid-cols-2 gap-4">
              ...
              {/* Étape 4 */}
              {etapeActuelle === 4 && (
                <div className="bg-teal-500 rounded-3xl p-8 text-white shadow-2xl">
                  <h2 className="text-2xl font-black mb-6 flex items-center gap-3"><Layers /> Supports</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {contenusSupports.map(c => (
                      <label key={c} className="flex items-center space-x-3 bg-white/10 p-4 rounded-lg cursor-pointer">
              ...
              {etapeActuelle === 5 && (
                <div className="bg-sunuBlue rounded-3xl p-8 text-white shadow-2xl">
                  ...
                  <div className="bg-white/10 p-6 rounded-2xl mb-6">
                    <p className="font-bold">{formData.prenom} {formData.nom}</p>
                    <p>{formData.email} | {formData.telephone}</p>
                    <p className="mt-4 text-sm opacity-80">{formData.projets.length} services sélectionnés</p>
                  </div>
                  <label className="flex items-start space-x-3 bg-white/10 p-4 rounded-xl cursor-pointer">
                    <Checkbox checked={formData.consentement} onCheckedChange={(v) => setFormData(p => ({...p, consentement: v as boolean}))} className="border-white" />
                    <span className="text-sm">J'accepte que SUNULINK CONSULTING utilise ces données pour mon devis.</span>
                  </label>
                </div>
              )}
              ...
              <div className="flex justify-between mt-8">
                {etapeActuelle > 1 && (
                  <Button type="button" onClick={precedent} variant="outline" className="rounded-xl px-8">
                    <ArrowLeft className="mr-2" /> Précédent
                  </Button>
                )}
                {etapeActuelle < 5 ? (
                  <Button type="button" onClick={suivant} disabled={!validateEtape(etapeActuelle)} className="bg-sunuBlue text-white rounded-xl px-8 ml-auto">
                    Suivant <ArrowRight className="ml-2" />
                  </Button>
                ) : (
                  <Button type="submit" disabled={!formData.consentement || isSubmitting} className="bg-sunuOrange text-white rounded-xl px-8 ml-auto">
